...
async function performGeminiQuery(hostname) {
    ...
    try {
        ...
        // Grounding Metadata Processing. URLs generated by the model are often
        // not valid, so we try to find a match in the grounding metadata.
        const groundingMetadata = candidate?.groundingMetadata;
        if (groundingMetadata) {
            console.log("Grounding Metadata:", groundingMetadata);

            const groundingCandidates = [];
            // Extract URI and Title from groundingChunks
            // Unfortunately the grounding URLs are always redirects, but the 
            // title tends to contain the domain they're about.
            ...
            // Process reviews and overwrite URLs
            if (jsonResult.reviews && Array.isArray(jsonResult.reviews)) {
                jsonResult.reviews.forEach(review => {
                    if (!review.url) return;

                    try {
                        // Normalize review URL hostname (remove www)
                        const reviewHostname = new URL(review.url).hostname.replace('www.', '').toLowerCase();

                        // Find grounding candidate where TITLE (treated as hostname) matches
                        const matchedCandidate = groundingCandidates.find(c => {
                            if (!c.title) return false;
                            const titleHostname = c.title.toLowerCase().replace('www.', '');
                            return titleHostname === reviewHostname;
                        });

                        if (matchedCandidate && matchedCandidate.uri !== review.url) {
                            console.log(`Updating URL for ${review.source}: \n[Old] ${review.url}\n[New] ${matchedCandidate.uri}`);
                            review.url = matchedCandidate.uri;
                        }
                    } catch (e) { /* Invalid review URL */ }
                });
            }
        }
